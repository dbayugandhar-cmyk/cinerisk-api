    ...
    async with httpx.AsyncClient(timeout=12, follow_redirects=True) as client:
        try:
            ...
            if r.status_code == 200:
                body = r.text.lower()
                found = [k for k in ["cam", "telesync", "ts ", "torrent", "download"] if k in body]
                if len(found) >= 2:
                    results["hits"] += 1
...
Confidence:  {conf:.0%}
Gap:         {gap_minutes} min ({gap_minutes/60:.1f} hours before CAM found online)
Platforms:   {', '.join(scan['platforms']) or 'Unknown'}
First URL:   {scan['first_url'] or 'N/A'}
Total hits:  {scan['hits']}
...
"""
    ...
    if not SENDGRID_API_KEY:
        print("[LAYER4] No SENDGRID_API_KEY - email skipped, logged above")
        return
    subject = f"[CINEOS] CAM confirmed - {film} - {gap_minutes}min gap - {zone} zone"
    async with httpx.AsyncClient(timeout=10) as client:
        try:
            ...
            print(f"[LAYER4] Email {'sent OK' if r.status_code == 202 else f'error {r.status_code}'}")
        except Exception as e:
...
async def run_scan_and_alert(incident_id: str, film_title: str,
                              ...
                              detected_at, confidence: float, db):
    if confidence < SCAN_THRESHOLD:
        return
    film = film_title.strip() if film_title and film_title not in ("string", "") else None
    if not film:
        return
    ...
    try:
        ...
        print(f"[LAYER4] DB write error: {e}")
    if scan["hits"] > 0:
        try:
            await db.execute("UPDATE incidents SET alerted = true WHERE id = $1", incident_id)
        except Exception as e:
            print(f"[LAYER4] alerted update error: {e}")
        ...
        print(f"[LAYER4] No CAM found yet - scheduling re-scan in 2h for '{film}'")
        async def rescan_later():
            await asyncio.sleep(7200)
            print(f"[LAYER4] Re-scanning '{film}' (2h check)...")
            ...
            if scan2["hits"] > 0:
                try:
                    ...
                    print(f"[LAYER4] re-scan error: {e}")
        asyncio.create_task(rescan_later())
